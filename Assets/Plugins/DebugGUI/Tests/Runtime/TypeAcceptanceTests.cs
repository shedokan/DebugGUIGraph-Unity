...
public class TypeAcceptanceTests
{
    ...
    public IEnumerator TearDown()
    {
        ...
        yield return null; // Wait for Destroy to process before rescanning
    ...
    // [Test] (not [UnityTest]) — NUnit 3 rejects IEnumerator return type on [TestCaseSource] methods.
    // [UnitySetUp]/[UnityTearDown] still run as coroutines; the test body itself is synchronous.
    [Test, TestCaseSource(nameof(SupportedFieldCases))]
    public void Field_IsSupported(Type behaviourType)
    {
        CreateWithComponent(behaviourType);
        LogAssert.NoUnexpectedReceived();
    }

    [Test, TestCaseSource(nameof(SupportedPropertyCases))]
    public void Property_IsSupported(Type behaviourType)
    {
        CreateWithComponent(behaviourType);
        LogAssert.NoUnexpectedReceived();
    }

    [Test, TestCaseSource(nameof(SupportedMethodCases))]
    public void Method_IsSupported(Type behaviourType)
    {
        CreateWithComponent(behaviourType);
        LogAssert.NoUnexpectedReceived();
    }

    [Test, TestCaseSource(nameof(UnsupportedCases))]
    public void Unsupported_IsRejected(Type behaviourType)
    {
